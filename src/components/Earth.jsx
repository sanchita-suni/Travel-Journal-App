...
export default function Earth() {
  ...
  useEffect(() => {
      ...
      .then((geo) => {
        geo.features.forEach((f) => {
          ...
          if (f.geometry.type === "MultiPolygon")
            f.geometry.coordinates.forEach((p) => drawPolygon(p));
        });
      });
    ...
    async function pinPlace() {
      ...
      const code =
        info.address?.country_code?.toLowerCase() || "un";
    ...
    function onClick(event) {
      ...
      const intersects = raycaster.intersectObjects(
        pinnedFlags.map((p) => p.flag)
      );

      if (intersects.length > 0) {
        const hit = pinnedFlags.find(
          (p) => p.flag === intersects[0].object
        );

        if (hit) {
          const weather = hit.weather
            ? `${Math.round(
                hit.weather.main.temp
              )}°C, ${hit.weather.weather[0].description}`
